...
if $UPDATE_ONLY; then
  ...
  cp "$SCRIPT_DIR/orchestrator.sh" "$SCRIPTS_DIR/gwym-agent.sh"
  chmod +x "$SCRIPTS_DIR/gwym-agent.sh"
  cp "$SCRIPT_DIR/gwym-agent.conf.default" "$SCRIPTS_DIR/gwym-agent.conf.default"
...
if [[ -f "$SCRIPTS_DIR/gwym-agent.sh" ]]; then
  echo "Updating: $SCRIPTS_DIR/gwym-agent.sh"
else
  echo "Installing: $SCRIPTS_DIR/gwym-agent.sh"
fi
cp "$SCRIPT_DIR/orchestrator.sh" "$SCRIPTS_DIR/gwym-agent.sh"
chmod +x "$SCRIPTS_DIR/gwym-agent.sh"
...
if [[ ! -f "$SCRIPTS_DIR/gwym-agent.conf" ]]; then
  cp "$SCRIPT_DIR/gwym-agent.conf.default" "$SCRIPTS_DIR/gwym-agent.conf"
  echo "Created config: $SCRIPTS_DIR/gwym-agent.conf"
  echo "  Edit this file to set your project-specific values (GITHUB_REPO, TEST_CMD, etc.)"
else
  echo "Config already exists: $SCRIPTS_DIR/gwym-agent.conf (not overwritten)"
fi
...
# Agent Dashboard launcher — auto-generated by gwym-agent/install.sh
...
declare -a IGNORE_ENTRIES=(
  ".claude/worktrees/"
  ".claude/agent-memory/"
  ".claude/scripts/gwym-agent.conf"
  ".claude/assets/"
)
...
echo ""
echo "Next steps:"
echo "  1. Edit $SCRIPTS_DIR/gwym-agent.conf with your project settings"
