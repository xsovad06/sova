...
- **`init_db()` must register engine in `_engines` dict** -- `get_session(project_dir=...)` resolves the DB URL via `_get_database_url()` then looks it up in `_engines`. If `init_db()` only stores in global `_session_factory` but not `_engines`, `get_session(project_dir=...)` creates a second engine for the same URL -- silently splitting data across two in-memory DBs in tests. Fix: `_engines[url] = (_engine, _session_factory)` in `init_db()`. File: `sova/db/session.py`. PR #243. [confirmed: 0]
