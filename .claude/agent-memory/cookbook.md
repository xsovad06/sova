...
- **`MagicMock(name=...)` sets debug name, not `.name` attribute** -- `MagicMock(name="foo")` configures the mock's internal `_mock_name` used in `repr()`. The `.name` attribute becomes another `MagicMock` object, not the string `"foo"`. To set `.name`, create the mock first, then assign: `m = MagicMock(); m.name = "foo"`. This is critical for CI check mocks where `c.name` is used for substring matching in `exclude_checks`. File: `tests/test_core.py:TestMonitorCIStep`. [confirmed: 1]
...
- **`re.sub` equality guard fails when text already contains replacement value** -- pattern `updated = re.sub(pat, replacement, text); if updated == text: error("not found")` falsely reports "not found" when the text already has the replacement value (e.g., `**Status**: approved` replaced with `approved`). The substitution succeeds but produces identical output. Fix: use `re.search(pat, text)` to check existence first, then `re.sub()` to replace. File: `sova/core/steps/spec.py`. [confirmed: 1]
- **`ci.exclude_checks` filters checks by exact name match** -- `CIConfig.exclude_checks: list[str]` names are matched via `c.name not in excluded` after `get_ci_checks()` returns. `flaky_checks` uses the same exact match but only ignores failures (check still runs). Use for checks that should never block the pipeline (e.g., `bonfire-tekton` on Konflux). File: `sova/core/steps/monitor_ci.py`. [confirmed: 1]
