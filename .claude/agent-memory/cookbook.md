...
- **Update hardcoded Alembic head revision in tests after adding a migration** -- `test_get_alembic_head_returns_current_head` asserts a specific revision string (e.g., `"018"`). After adding migration 019, this test fails until the assertion is updated. Always update this test when a new migration is added. PR #354. [confirmed: 1]
