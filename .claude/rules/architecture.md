...
- 19 services: run, cost, memory, control (facade), handoff, queue, batch, work, task, log, settings, setup, agent_lifecycle, agent_output, agent_recovery, agent_handoff, agent_pool, agent_db, output (re-export facade for core/output)
...
- **Never `overflow: hidden` on sidebar nav**: the sidebar `<nav>` contains absolutely-positioned children (notification panel, CSS hover tooltips) that extend beyond its boundary at `left: 100%`. Adding `overflow: hidden` to the nav -- even conditionally during collapsed state -- clips these popouts. Use `max-width` and opacity transitions on individual child elements instead of overflow on the container. This applies to any fixed-position sidebar or panel that contains popout menus or tooltips.
