...
- **Pipeline variant detection must gate on `current_step`**: `get_step_progress()` and `_detect_variant()` use `role+pr_number` to identify address-review runs, but ONLY when `current_step in (None, "agent")`. The dashboard outer-process TaskRun uses `current_step="agent"` (set at creation, never updated by the WorkflowEngine). WorkflowEngine TaskRuns progress through real step names and acquire `pr_number` mid-pipeline via `_sync_task_run_context()` after `CreatePRStep` -- without the gate, every developer run that creates a PR would be mislabeled as "address_review" for steps 10-12 (monitor_ci, extract_memory, handoff_to_reviewer). `_detect_variant_from_steps()` is authoritative when step history contains address-review-only steps; otherwise falls back to `get_step_progress()`.
