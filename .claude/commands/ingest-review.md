...
3. **Fetch external review tool findings** (if configured):
   Read `sova.toml` for `[external_reviews]` configuration. If absent or `enabled = false`, skip this step.
...
6. Update `.claude/agent-memory/cookbook.md`:
   ...
   - For external tool findings, use the "External Review Tools" section. Include the tool name and rule key for traceability:
