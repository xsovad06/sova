...
- **`gh pr edit --title/--body` requires `read:org`/`read:discussion` scope, even for a title-only edit**: `gh pr edit` resolves suggested reviewers/labels via a GraphQL query that touches organization fields regardless of which flags you pass, so it fails the same way whether you edit title, body, or both together. The error text varies by `gh` version: sometimes the short `missing required scopes [read:org]`, sometimes a longer per-field GraphQL error naming `login`/`name`/`slug` fields and the scopes each requires. Either way, the fix is the same: use the REST API instead. `gh api -X PATCH repos/<owner>/<repo>/pulls/<number> -f title="..."` needs only `repo` scope. For the body, `-F body=@<file>` reads the value directly from a file (`-F`, capital, not `-f`) and handles multi-line markdown without any quoting or a separate `--input file.json` JSON wrapper: `gh api repos/<owner>/<repo>/pulls/<number> -X PATCH -F body=@/tmp/pr-body.md`. Combine title and body in one call by passing both flags together, or split into two calls if that is simpler to script.
...
- `toolsets` (default `["default"]`): mcp-atlassian's `TOOLSETS` env var; unrecognized names are silently ignored by the sidecar, so stick to documented toolset names (e.g. `default`, `all`, `jira_agile`)
...
- `atlassian_config_problems()` gates injection: an enabled sidecar with no URL, no credential, a non-HTTPS `jira_url`/`confluence_url` (the PAT or API token travels alongside the URL), or `auth_type = "api_token"` without an `email` is skipped with the missing/invalid keys listed, instead of writing an entry that cannot start
...
- Disabling the sidecar, or leaving it enabled with an invalid config, removes any previously written `.mcp.json` entry and revokes its `enabledMcpjsonServers` approval on the next `sova install`, so a stale sidecar (and any embedded token) does not linger
