# I Built an Autonomous Software Agent That Develops Features While I Sleep. Here's What 105 Issues Taught Me.
...
I'm a solo developer working on SOVA, a Python application with ~5,700 tests, CI via GitHub Actions, SonarCloud quality gates, and CodeRabbit automated review. Over the past few months, I've been running two parallel development tracks on the same codebase:
...
I developed 17 issues interactively with timestamped event logging, and 105 issues autonomously with full telemetry in the database. Same codebase. Same CI pipeline. Same model. Different levels of human involvement.

## The Numbers

### Cost: 18.5x cheaper autonomous

This was the headline finding. Interactive sessions averaged **$116.70** per issue (where cost was recorded, n=13). Autonomous runs averaged **$6.31** per issue across all 105 issues. The median autonomous cost was **$5.05**.

The entire autonomous fleet of 105 issues cost **$662.61 total**. That's less than six interactive sessions.
...
### Speed: 1.5x more active agent time interactive

Interactive sessions averaged **136 minutes** of active agent compute time per issue versus **90 minutes** autonomous. The autonomous pipeline doesn't waste time re-establishing context between commands, doesn't wait for human decisions, and doesn't serialize work through a single conversation thread.
...
### Throughput: 73,930 lines across 105 issues

The autonomous fleet produced 73,930 lines of code changes across 105 completed issues, averaging 710 LOC and 9 files per issue. These aren't toy tasks: they include dependency graph engines, fleet analytics dashboards, batch API submission paths, review automation, and supervisor-level orchestration.

### Reliability: comparable, with caveats

Both modes had low CI failure rates. Across the full autonomous fleet, there were 22 CI step failures across 105 issues and 62 failed developer runs (0.6 per issue). Interactive had zero failures across all 17 issues.

The failure rate is the price of autonomy. Without a human catching "that's not what I meant" early, some percentage of autonomous runs go down the wrong path. But at $6.31 per attempt, retrying is cheap. And 68% of autonomous issues completed with zero retries.

## What I Actually Do Now

The benchmark revealed something I didn't expect: **in interactive sessions, I'm idle 64% of the time**. I'm waiting for the agent to finish a step, reviewing output, deciding what to do next, or taking breaks. My actual contribution is decision-making at gates, not code generation.
...
**Sample size**: 17 interactive issues with 13 having cost data. The sample is larger now but still smaller than the autonomous set (105 issues). These numbers are directional, not definitive.
...
**Model difference**: Interactive sessions primarily used Opus 4.6 (more expensive). SOVA runs used the default CLI provider. This contributes to the cost gap, though context window economics explain most of the 18.5x difference.
...
The 18.5x cost difference is not an argument against interactive development. It's an argument for having both modes and knowing when to use each.

Autonomous development is not a replacement for engineering judgment. It's a multiplier. The 105 issues SOVA completed autonomously are issues I would have done interactively, one at a time, over months. Instead, they shipped in weeks, at a fraction of the cost, while I focused on the hard problems that actually needed human attention.
