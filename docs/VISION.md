...
2. **Standardized Skills/Commands**: 28 general-purpose commands (develop, test, review, pr, debug, etc.) that work on any project without customization.
...
4. **Dashboard App**: FastAPI web UI with 24 pages for monitoring, agent control, lifecycle tracking, supervisor, fleet management, and configuration.
...
sova/
  sova/                            # Python package (20 modules)
    ...
    db/                            # SQLAlchemy 2.0 async ORM (25 models), Alembic migrations
    utils/                         # Logging, shell, formatting
    awareness/                     # Awareness subsystem (briefing, provider registry)
  ...
  tests/                           # pytest suite (6,400+ test functions)
...
### Developer Pipeline (16 steps)
Sync -> Assess -> CreateWorktree -> CaptureBaseline -> Develop -> Simplify -> SelfReview -> Commit -> Validate -> Push -> CreatePR -> WaitForExternalReviews -> AddressExternalFindings -> MonitorCI -> ExtractMemory -> HandoffToReviewer

### Address-Review Pipeline (10 steps)
EnsureWorktree -> Rebase -> AddressReview -> RearrangeCommits -> Validate -> Push -> MonitorCI -> ResolveExternalReviews -> ExtractMemory -> HandoffToUser

### Researcher Pipeline (4 steps)
FetchTask -> Research -> Spec -> ExtractMemory

### Planner Pipeline (4 steps)
ScanProject -> GenerateTasks -> ValidateTasks -> ExtractMemory
...
- **Supervisor**: task progression engine with dependency graph and pending actions
...
- **PR Metrics**: pull request timeline and review analysis
- **Queue**: batch operations (triage/harden multiple issues), progress bar
- **Specs**: spec management and approval workflow
- **Logs**: real-time agent output streaming
- **Memory**: knowledge base browser (memories, patterns, extractions)
- **Fleet**: fleet overview for multi-machine deployments
- **Oversight**: automated oversight agent findings
...
### TaskAdapter ABC (20 abstract methods)
...
| PatternFly in dependencies | `patternfly.md` | PatternFly components, layout patterns | Done |
| Generic Python project | `python.md` | Python conventions, packaging, testing | Done |
...
**Status: DONE** (28 standardized commands)

The command library is unified and portable across projects:
- 28 standardized commands in `commands/` (general-purpose, work for both interactive Claude Code and the autonomous agent)
...
Each browser tab is a fully independent workspace with its own agent status, costs, logs, and control panel. Tabs don't interfere with each other because config resolution is per-request (Python contextvars), not global state.
...
1. **Home page as command center**: cross-project summary with running agents, pending checkpoints, total daily cost, recent activity across all projects. Currently a project list; expanding to a full overview.
...
| Post-phase | Hardening | Handoff actions, multi-project, batch ops, design system, lifecycle, supervisor, fleet, oversight, 6,400+ tests |
...
| #20 | Tag v0.1.0 release | In Progress |
...
| # | Feature | Description |
|---|---------|-------------|
| ~~#33~~ | ~~VM deployment + always-on mode~~ | ~~systemd service hardening, notification abstraction, hybrid laptop+VM~~ (done: #705) |
