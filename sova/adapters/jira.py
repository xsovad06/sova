...
def _is_exact_identity_match(query: str, result: dict) -> bool:
    """Check whether a Jira ``/user/search`` result is an exact identity match for ``query``.

    ``/user/search`` matches on a prefix basis, so this narrows a possibly broad
    candidate list down to results that exactly equal the query (case-insensitive)
    on email, email local-part, or display name, closing off collisions where the
    query is a substring of an unrelated user's identity.
    """
    query_norm = query.strip().lower()
    if not query_norm:
        return False
    email = str(result.get("emailAddress") or "").lower()
    email_local = email.split("@", 1)[0]
    display_name = str(result.get("displayName") or "").lower()
    return query_norm in {email, email_local, display_name} - {""}
...
class JiraAdapter(TaskAdapter):
    ...
    async def _resolve_account_id(self, query: str) -> str | None:
        ...
        ``/user/search`` matches on a prefix basis, so a partial or colliding
        query can return an unrelated user. Requesting multiple candidates and
        keeping only exact matches (against email local-part, full email, or
        display name) rejects an ambiguous result instead of silently
        assigning the first prefix match.
        """
        try:
            response = await self._http.get(
                "/user/search", params={"query": query, "maxResults": _MAX_USER_SEARCH_RESULTS}
            )
        ...
        try:
            ...
            return None
        exact_matches = [r for r in results if _is_exact_identity_match(query, r)]
        if len(exact_matches) != 1:
            log.warning(
                "resolve_account_id.ambiguous_or_no_exact_match",
                query=query,
