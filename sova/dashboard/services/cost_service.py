...
async def get_summary(session: AsyncSession) -> dict:
    ...
    return {
        "total_cost_usd": round(float(total or 0), 4),
        "total_invocations": count,
        "today_cost_usd": round(float(today_total or 0), 4),
        "rolling_7d_usd": round(float(rolling_7d or 0), 4),
    }
...
async def get_daily(session: AsyncSession, days: int = 14) -> list[dict]:
    ...
    return [{"date": str(row.day), "cost_usd": round(float(row.cost or 0), 4)} for row in result.all()]
