...
Once published, install from the marketplace listing shown in `/plugin`:

```bash
/plugin install sova@<marketplace-name>
```

Replace `<marketplace-name>` with the name shown for this plugin in your `/plugin` listing (e.g. the marketplace it was installed from), since it may differ from the plugin's own name (`sova`).
