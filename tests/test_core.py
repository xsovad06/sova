...
class TestWorkflowJournalIntegration:
    async def test_journal_created_on_run(self) -> None:
        ctx = _make_ctx()
        step = DummyStep(should_pass=True, gate_pass=True)
        engine = WorkflowEngine(steps=[step], ctx=ctx)
        ...
        assert result.success
        journal_path = ctx.project_dir / ".claude" / "runs" / str(result.task_run_id) / "journal.jsonl"
    ...
    async def test_journal_records_failure(self) -> None:
        ctx = _make_ctx()
        step = DummyStep(should_pass=False, gate_pass=True)
        engine = WorkflowEngine(steps=[step], ctx=ctx)
    ...
    async def test_journal_records_gate_failure(self) -> None:
        ctx = _make_ctx()
        step = DummyStep(should_pass=True, gate_pass=False)
        engine = WorkflowEngine(steps=[step], ctx=ctx)
    ...
    async def test_per_issue_budget_exceeded_closes_journal(self) -> None:
        config = ProjectConfig(agent={"max_issue_budget": Decimal("0.01")})
        ctx = _make_ctx(config=config)
        ...
        assert not result.success
        assert result.final_status == TaskStatus.PAUSED
        assert "Per-issue budget exceeded" in result.error


class TestWorkflowUnknownErrorConstant:
    async def test_unknown_error_used_when_record_result_is_none(self) -> None:
        ctx = _make_ctx()
        step = DummyStep(should_pass=False, gate_pass=True)
        step.max_retries = 0

        async def failing_execute(ctx_inner: ExecutionContext) -> StepResult:
            return StepResult(success=False, summary="Failed", error="specific error")

        step.execute = failing_execute
        engine = WorkflowEngine(steps=[step], ctx=ctx)
        result = await engine.run()
    ...
    async def test_unknown_error_in_failure_record(self) -> None:
        ctx = _make_ctx()
        step = DummyStep(should_pass=False, gate_pass=True)
        step.max_retries = 0
        engine = WorkflowEngine(steps=[step], ctx=ctx)
        ...
        assert not result.success
        async with await get_session() as session:
            stmt = select(FailureRecord).where(FailureRecord.task_run_id == result.task_run_id)
            failures = (await session.scalars(stmt)).all()
