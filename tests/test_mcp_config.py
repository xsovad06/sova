...
def test_inject_creates_mcp_json_file(tmp_path: Path) -> None:
    ...
    assert result is MCPWriteResult.CHANGED
...
def test_inject_idempotent(tmp_path: Path) -> None:
    ...
    (tmp_path / ".mcp.json").write_text(json.dumps(existing))
    result = inject_mcp_server(tmp_path, "test-mcp", _TEST_SERVER)
    assert result is MCPWriteResult.UNCHANGED
...
def test_inject_private_restricts_permissions(tmp_path: Path) -> None:
    ...
    assert inject_mcp_server(tmp_path, "test-mcp", _TEST_SERVER, private=True) is MCPWriteResult.CHANGED
    assert (tmp_path / ".mcp.json").stat().st_mode & 0o777 == 0o600


def test_inject_private_restricts_permissions_on_unchanged_entry(tmp_path: Path) -> None:
    """A matching entry written non-privately before still gets locked down on a private re-run."""
    (tmp_path / ".mcp.json").write_text(json.dumps({"mcpServers": {"test-mcp": _TEST_SERVER}}))
    (tmp_path / ".mcp.json").chmod(0o644)
    result = inject_mcp_server(tmp_path, "test-mcp", _TEST_SERVER, private=True)
    assert result is MCPWriteResult.UNCHANGED
    assert (tmp_path / ".mcp.json").stat().st_mode & 0o777 == 0o600


def test_inject_private_unchanged_entry_reports_error_on_chmod_failure(tmp_path: Path) -> None:
    """A matching entry that cannot be locked down reports ERROR rather than a false UNCHANGED."""
    (tmp_path / ".mcp.json").write_text(json.dumps({"mcpServers": {"test-mcp": _TEST_SERVER}}))
    with patch.object(Path, "chmod", side_effect=OSError("permission denied")):
        result = inject_mcp_server(tmp_path, "test-mcp", _TEST_SERVER, private=True)
    assert result is MCPWriteResult.ERROR


def test_inject_private_handles_oserror_and_cleans_temp_file(tmp_path: Path) -> None:
    """A failed private write reports ERROR and leaves no leftover temp file."""
...
def test_approval_added_to_empty_settings(tmp_path: Path) -> None:
    assert set_project_mcp_approval(tmp_path, "test-mcp", approved=True) is MCPWriteResult.CHANGED
    settings = json.loads((tmp_path / "settings.json").read_text())
...
def test_approval_idempotent(tmp_path: Path) -> None:
    ...
    assert set_project_mcp_approval(tmp_path, "test-mcp", approved=True) is MCPWriteResult.UNCHANGED
...
def test_approval_revoked(tmp_path: Path) -> None:
    ...
    assert set_project_mcp_approval(tmp_path, "test-mcp", approved=False) is MCPWriteResult.CHANGED
    settings = json.loads((tmp_path / "settings.json").read_text())
...
def test_approval_revoke_when_absent(tmp_path: Path) -> None:
    assert set_project_mcp_approval(tmp_path, "test-mcp", approved=False) is MCPWriteResult.UNCHANGED
...
def test_approval_skips_malformed_key(tmp_path: Path) -> None:
    ...
    assert set_project_mcp_approval(tmp_path, "test-mcp", approved=True) is MCPWriteResult.ERROR
...
def test_atlassian_problems_missing_urls_and_token(monkeypatch) -> None:
...
def test_atlassian_problems_rejects_http_urls() -> None:
    cfg = AtlassianMCPConfig(enabled=True, jira_url="http://issues.redhat.com", auth_type="pat", token="t")
    problems = atlassian_config_problems(cfg)
    assert any("jira_url" in p and "https" in p for p in problems)


def test_atlassian_problems_accepts_https_urls() -> None:
    cfg = AtlassianMCPConfig(
        enabled=True,
        jira_url="https://issues.redhat.com",
        confluence_url="https://issues.redhat.com/wiki",
        auth_type="pat",
        token="t",
    )
    assert atlassian_config_problems(cfg) == []
